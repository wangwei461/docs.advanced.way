...
export default {
  ...
  render(
    ...
    // for tocSidebar: auto pages, a hash link should be active if one of its child
    // matches
    const active =
      item.type === "auto"
        ? selfActive ||
    ...
    const maxDepth = [
      $page.frontmatter.tocSidebarDepth,
      tocSidebarDepth,
      $themeLocaleConfig.tocSidebarDepth,
      $themeConfig.tocSidebarDepth,
      3,
...
function renderLink(h, to, text, active, level) {
  const component = {
    ...
    class: {
      active,
      "tocSidebar-link": true,
    },
  };
  if (level == 1) {
    component.style = {
      "padding-left": level * 0.9 + "rem",
      "font-weight": "500",
    };
  } else if (level == 2) {
    component.style = {
      "padding-left": level * 0.4 + "rem",
      "font-weight": "400",
    };
  }
...
function renderChildren(h, children, path, route, maxDepth, depth = 1) {
  ...
  return h(
    "ul",
    { class: "tocSidebar-sub-headers" },
    children.map((c) => {
      const active = isActive(route, path + "#" + c.slug);
      return h("li", { class: "tocSidebar-sub-header" }, [
...
<style lang="stylus">
.tocSidebar .tocSidebar-sub-headers {
  padding-left: 1rem;
  font-size: 0.95em;
}
